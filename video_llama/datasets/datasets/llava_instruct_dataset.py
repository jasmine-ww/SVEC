...
class Instruct_Dataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_root,num_video_query_token=32,tokenizer_name = '/mnt/workspace/ckpt/vicuna-13b/',data_type = 'image', model_type='vicuna', vis_root2="", num_frm=16):
        """
        ...
        for item in annotation:
            if "image" in item and "ocr_vqa" not in item['image']:
    ...
    def _get_image_path2(self, sample):
        rel_video_fp = sample['image']
        full_video_fp = os.path.join(self.vis_root,  rel_video_fp)
        return full_video_fp
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            try:
                ...
                # if ("image" not in sample) or "ocr_vqa" in sample['image']:
                #     if _ > 0:
                #         _ -= 1
                #     continue   #jsm


                image_path = self._get_image_path2(sample)   #jsm
