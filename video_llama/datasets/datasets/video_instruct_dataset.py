...
class Video_Instruct_Dataset(BaseDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_root,num_video_query_token=32, num_frm=8, tokenizer_name = '/mnt/workspace/ckpt/vicuna-13b/',data_type = 'video', model_type='vicuna', vis_root2=""):
        """
    ...
    def _get_video_path(self, sample):
        if self.vis_root.split('/')[-1] == "activitynet":
            rel_video_fp = sample['video']
            path = os.path.join(self.vis_root,"rawvideos_train/"+rel_video_fp+".mp4")
            path1 = os.path.join(self.vis_root,"rawvideos_val/"+rel_video_fp+".mp4")
            path2 = os.path.join(self.vis_root,"rawvideos_test/"+rel_video_fp+".mp4")
            if os.path.exists(path):
                ...
                return path2
        rel_video_fp = sample['video']
        page_dir = rel_video_fp.split('/')[0]
        if int(page_dir.split('_')[0]) < 120:
            full_video_fp = ""
        if int(page_dir.split('_')[0]) < 14852 and int(page_dir.split('_')[0]) > 60:
            full_video_fp = os.path.join(self.vis_root,  rel_video_fp)
        else:
            full_video_fp = os.path.join(self.vis_root2,  rel_video_fp)
        # full_video_fp = os.path.join(self.vis_root,  rel_video_fp)
    ...
    def collater(self, instances):
        ...
        if 'image' in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch['images'] = torch.stack(images)  #batch['images']
            else:
                batch['images'] = images  #batch['images']
