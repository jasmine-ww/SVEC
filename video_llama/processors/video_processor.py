...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False, video_id="", middle = 0):
    decord.bridge.set_bridge("torch")
    ...
    elif sampling == "breakpoint":
        # num_half = min(n_frms/2, middle)
        # num_half = min(num_half, vlen-middle)
        # s_ = middle-num_half
        # indices = np.arange(s_, s_ + n_frms)

        e = min(middle+n_frms-1,vlen-1)
        indices = np.arange(middle, e)
